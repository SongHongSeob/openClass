 ...
 * /api/waitlist-entries/{entryId}}). 인증은 이 컨트롤러가 아니라 {@code
 * SecurityConfig}의 {@code anyRequest().authenticated()} 기본 규칙이 처리한다
 * (REQ-WL-008; {@code SecurityConfig.java}는 이 SPEC의 PRESERVE 대상이라 새
 * 인가 규칙을 추가하지 않는다).
 */
@RestController
