...
## §J UNVERIFIED 15건 실관측 (3차, 2026-08-30) + SPEC-AUTH-001 결함 발견·수정

> `§H.3`이 미검증으로 남긴 `[수동]` AC 15건 중 12건을 오케스트레이터가 직접
> `claude-in-chrome`으로 실관측했다. 이 과정에서 SPEC-FRONTEND-001과 무관한
> **SPEC-AUTH-001의 실제 프로덕션 결함**(F13)을 발견해 별도 제자리 개정으로
> 수정했다(`main` 커밋 `e40ad0f`). 3건(AC-FE-072, 073b, 110의 확정 목록 절반)은
> 이번 회차에서도 미관측으로 남는다 — 아래 J.5 참고.

### J.1 환경 이슈 — 로컬 포트 충돌 (wedding_project)

로컬 5173 포트를 무관한 다른 프로젝트(`wedding_project`)의 Vite 개발 서버가
선점하고 있어, 이 SPEC의 프론트엔드를 5175 포트로 재기동하고 백엔드
`CORS_ALLOWED_ORIGINS`도 그에 맞춰 재설정했다(사용자 승인 하에 진행). 코드
변경 없음 — 순수 로컬 실행 환경 조정.

### J.2 F13 (신규, Blocker급 실사용 결함) — `/error`가 permitAll 목록에서 누락 → 모든 MVC 해석 오류가 401로 가려짐

AC-FE-021(회원가입 400) 재검증 도중 발견. 짧은 비밀번호로 회원가입을
제출하면 기대되는 400 대신 **401(빈 본문)**이 돌아왔고, 프론트엔드의 전역
401 처리기가 이를 "세션 만료"로 오인해 방문자에게 완전히 엉뚱한 안내를
표시했다.

**근본 원인**: `SecurityConfig.authorizeHttpRequests`가 `/error`를 permitAll하지
않아, `DefaultHandlerExceptionResolver`가 검증 실패를 400으로 해석한 뒤
서블릿 컨테이너가 내부적으로 수행하는 `/error` 재전달이 `anyRequest().authenticated()`에
걸려 401로 거부되었다 — 실제 400 응답 본문이 클라이언트에 도달하기 전에
가로채인 것이다. `MockMvc` 기반 기존 테스트(`SignupIntegrationTest`)는 이
내부 재전달을 재현하지 않아 이 결함을 잡지 못했다.

**수정**: SPEC-AUTH-001을 제자리 개정(`completed → in-progress → completed`,
`amendment_of: SPEC-AUTH-001`)해 `SecurityConfig.java`에 `.requestMatchers("/error").permitAll()`
한 줄 추가. 실제 서버 환경(`@SpringBootTest(webEnvironment = RANDOM_PORT)` +
`TestRestTemplate`, MockMvc 아님)으로 회귀 가드 테스트 4건 신설 — 수정 전
전부 FAIL(401), 수정 후 전부 PASS(400). 전체 테스트 스위트 재실행, 관련
엔드포인트 2건 추가 스팟체크(로그인 malformed JSON, 관리자 강좌 생성 검증
실패) 모두 정상화 확인. 상세 근거는 `.moai/specs/SPEC-AUTH-001/progress.md`
"Amendment 1" 절 참고. 커밋: `main` `e40ad0f` (푸시 완료), 이 SPEC 브랜치에도
병합 반영(`9775f9a`).

**프론트엔드 영향 없음**: SPEC-FRONTEND-001의 전역 401 처리기 자체는 정상
동작이었다 — 백엔드가 진짜 401을 잘못 보낸 것이 원인이었으므로 프론트엔드
코드는 무변경.
...
| AC | 판정 | 근거 |
|---|---|---|
| AC-FE-021 | **PASS** | F13 수정 후 실제 400 + "요청 내용을 다시 확인해 주세요." 정상 표시 확인(`minLength` 클라이언트 검증을 임시 우회해 서버 응답 경로까지 관측). 단, 정상 UI 경로에서는 `minLength=8`이 이 시나리오 자체를 브라우저 차원에서 도달 불가능하게 막는다(결함 아님 — 더 엄격한 방어) |
| AC-FE-022 | **PASS** | 동일 이메일 재가입 시도 → "이미 가입된 이메일입니다." 정상 표시 |
| AC-FE-024 | **PASS** | 틀린 비밀번호 로그인 → "이메일 또는 비밀번호가 올바르지 않습니다." (401, 원인 미구분) |
| AC-FE-025 | **PASS(간접)** | 인증 필요 엔드포인트(`/api/enrollments/mine`)가 200으로 통과 — Bearer 헤더 부착의 강한 방증. 헤더 원문 직접 관측은 도구 제약으로 못함(Gap, J.5) |
| AC-FE-029 | **PASS** | 로그아웃 클릭 → `sessionStorage` 토큰 실제 제거 확인 → 이후 `/enrollments/mine` 직접 진입 시 `/`로 리다이렉트 |
| AC-FE-034 | **PASS** | AC-FE-029와 동일 관측이 그대로 충족(비세션 상태 보호 라우트 직접 진입 차단) |
| AC-FE-041 | **PASS** | "다음" 클릭 → 실제 2페이지 항목(S11·M4재시도)으로 교체, "다음" 버튼 마지막 페이지에서 비활성화 |
| AC-FE-046 | **PASS** | 실제 백엔드 404(`COURSE_NOT_FOUND`)를 받는 상세 화면 진입 시 "존재하지 않는 강좌입니다." 정상 표시, 화면 중단 없음 |
| AC-FE-068 | **PASS** | 접수 시각을 30초 이전으로 강제 조정 후 재진입 → "처리가 예상보다 오래 걸리고 있습니다. 자동 확인이 중단되었습니다" + "다시 확인" 버튼 정상 노출 |
| AC-FE-069 | **PASS** | "다시 확인" 클릭 → 동일 `requestId`(23)로 `GET` 재조회만 발생, 신규 요청 생성 없음(네트워크 로그 확인) |
| AC-FE-105 | **PASS** | 타인 소유 대기명단 항목 취소 시도(404) vs 존재하지 않는 ID 취소 시도(404) — 응답 코드·메시지 패턴 완전 동일, 구분 불가 확인(curl 직접 대조) |
| AC-FE-110 | **PASS(부분)** | 대기명단 쪽: API 응답 순서(S5→S2, 둘 다 2번째)와 화면 표시 순서가 정확히 일치 확인. 확정 목록(`enrollmentId` 오름차순) 쪽은 미관측(Gap, J.5) |
| AC-FE-112 | **PASS** | 한 계정이 서로 다른 두 강좌(S5·S2)에 대기 중인 상태에서 "내 대기명단" 진입 → 각 항목이 `courseTitle`과 `position`을 나란히 표시("이 강좌 내 대기 2번째 (다른 강좌의 대기와는 무관한 강좌별 순번)"), 전역 순위로 오독될 표현 0건 |

### J.4 재현 중 발견한 감사자 방법론 아티팩트 (버그 아님 — 기록용)

- **claude-in-chrome 자동화 탭의 `document.visibilityState`가 상시 `hidden`** — `§H.10`이 이미 기록한 M7 S9의 동일 제약이 M4 폴링 재검증에서도 그대로 재현됐다. TanStack Query의 `refetchInterval`이 `hidden` 상태에서 백그라운드 재조회를 쉬는 것이 원인 — `Object.defineProperty`로 `visibilityState`를 `visible`로 임시 패치하자 폴링이 즉시 정상 재개되고 WAITLISTED 종단 상태까지 정확히 도달했다. **이 발견 자체가 M4 폴링 로직이 실제로 올바르게 구현되어 있음을 재확인**한다(문제는 검증 환경에 있었지 앱에 있지 않았다).
- **좌표 기반 클릭의 간헐적 미등록** — 이번 세션 초반 여러 차례 "가입/로그인이 조용히 실패"로 보였던 현상은 실제로는 `computer` 도구의 좌표 클릭이 React 리렌더 타이밍과 어긋나 클릭이 등록되지 않은 테스트 아티팩트였다(`curl`로 실제 계정 존재 여부를 대조해 확인). `find` 도구로 얻은 `ref` 기반 클릭으로 전환한 뒤에는 100% 안정적으로 재현됐다. 이 세션에서 초기에 "성공한 것처럼 보였던" 일부 화면(예: 최초 `acfe022` 가입 시도)은 실제로는 요청이 전송되지 않았을 가능성이 있다 — 이후 동일 항목을 `ref` 기반으로 재검증해 모두 PASS로 확정했으므로 최종 판정에는 영향 없다.

### J.5 미검증(Gap) — 이번 회차에도 관측하지 못한 것

- **AC-FE-072(언마운트 시 잔여 타이머 0건)**: 브라우저 뒤로가기(SPA 내부 전환)로 컴포넌트를 언마운트한 뒤 폴링 재개 여부를 확인했으나, 방법론상 하드 네비게이션과 완전히 분리해 단정하기 어려웠다 — 정황 증거(언마운트 후 추가 요청 미관측)는 있으나 React `useEffect` cleanup 자체를 격리 증명하지는 못했다.
- **AC-FE-073b(3초 간격 재개, 정확한 간격 수치)**: 새로고침 후 폴링이 재개되는 것 자체는 관측했으나("접수 시각" 기준점이 `sessionStorage`에서 정확히 복원됨을 J.3의 30초 상한 테스트가 간접 증명), "정확히 3초 간격"이라는 수치 자체를 스톱워치 수준으로 측정하지는 않았다.
- **AC-FE-110의 확정 목록(`enrollments/mine`) 절반**: 대기명단 쪽만 검증했다. 확정 목록이 `enrollmentId` 오름차순으로 응답과 동일하게 표시되는지는 이번 회차에서 별도 계정에 2건 이상의 확정 신청을 만들지 않아 미관측이다.
- **AC-FE-025의 헤더 원문**: `read_network_requests` 도구가 요청 헤더 본문을 노출하지 않아, "200 응답"이라는 간접 증거로 대체했다(§J.3 참고).

### J.6 잔여 정리 항목 (코드 변경 아님)

- 이번 회차에서 생성한 진단용 테스트 계정(`acfe0*@local.test`, `purecurl-test@local.test`)과 강좌 신청·대기 데이터가 로컬 dev DB에 남아 있다 — `§H.10`이 이미 기록한 M7 패턴과 동일하게 프로덕션 영향 없음, 정리는 별도 작업.
- 로컬 개발 서버(프론트엔드 `:5175`, 백엔드 `:8080`)가 이 세션 종료 시점에도 계속 실행 중일 수 있다 — 필요 시 종료.
