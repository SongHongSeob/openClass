...
**브랜치 상태에 관한 사실**: `SPEC-ENROLLMENT-001`의 구현(`enrollment`·`waitlist` 패키지, 29개 파일)은 `main`에 **아직 병합되지 않았다.** 브랜치 `sync/SPEC-ENROLLMENT-001`에 존재하며 PR로 열려 있다. 따라서 이 SPEC의 구현 시점에 해당 API가 `main`에 존재할지는 병합 여부에 달려 있다 — `plan.md` **§B.3**이 이를 선행 조건(`DEP-3` 진입 게이트)으로 다룬다.
...
3. **`sync/SPEC-ENROLLMENT-001`의 `main` 병합 시점** — PR #1이 열려 있으나 병합 여부와 시점은 이 SPEC의 통제 밖이다. **0.2.0 시점에 이것이 유일하게 남은 차단 요소(`DEP-3`)이며, 코드 가용성뿐 아니라 `depends_on` 사전 점검 게이트까지 막는다** — `plan.md` §B.3이 그 게이트의 성격과 선택지를 다룬다.
