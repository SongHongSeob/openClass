...
// 라우터는 아직 배선하지 않는다 — M2는 화면 간 이동을 이 컴포넌트의 지역
// 상태(useState)로 처리하며, 실제 다중 경로 라우팅(React Router, plan.md
// §C.6)은 카탈로그·수강신청 등 다중 화면이 실제로 필요해지는 이후 마일스톤이
// 도입한다. 인증 가드(routing/guards.tsx)는 이 화면에서 아직 사용하지 않는다
// — M2 자체의 화면(회원가입·로그인)은 둘 다 공개 화면이기 때문이다.
